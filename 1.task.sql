...
FROM (
	SELECT 
			...
			ROUND(AVG(v1.average_wages),2) AS avg_wages,
			ROUND(AVG(v2.average_wages),2) AS wages_prev,
			ROUND(((AVG(v1.average_wages)-AVG(v2.average_wages))/AVG(v2.average_wages))*100,2) AS growth
